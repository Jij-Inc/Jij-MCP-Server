"""JijModeling Grammer Guide Prompts"""

jm_placeholder_guide = """# Placeholder
```
...
jm_decision_variable_guide = """# Decision Variables
```
# shape of array parameter
n = d.len_at(0, latex="n")  # n is the length of the first dimension of d
m = d.len_at(1, latex="m")  # m is the length of the second dimension of d
...
jm_sum_guide = """# summation and element guide
...
jm_objective_func_guide = """# Objective Function
...
jm_constraint_guide = """# How to implement constraints in JijModeling
...
Caution:
- You cannot use Python loops directly in JijModeling For example: `for i in range(n):` is not allowed.
- Use `jm.sum()` with Element objects for any summations.
- Use `jm.Constraint` to define constraints with proper mathematical expressions.
"""


jijmodeling_guide_prompt = (
    "JijModeling Guides\n"
    + jm_placeholder_guide
    + jm_decision_variable_guide
    + jm_sum_guide
    + jm_objective_func_guide
    + jm_constraint_guide
)
